...
def clean_pt(pt):
    ...
    res = []
    for pos in pt:
        if pos[2] not in sw:#in ['NN','NNP','VB', 'PRP']:
            res.append(pos)
    return res
...
#array of tuple of (id, string) and keyword is query
def general_search(sample, keyword):
    grades = []
    for sent in sample:
        grade = search_it(sent[1], keyword)#1 because 0 is for id and 1 is for the string
        grades.append([sent,grade])
    grades.sort(key = lambda x: x[1], reverse=True)
    grades = filter(lambda x: x[1] > 0.5, grades) 
    return [lg[0] for lg in list(grades)]

def generate_detail(filename):
    regex = r"[0-9]+[ ][0-9][0-9.,]+"
    regex2 = r" pada [A-Z][^.,]+[.,]"
    bucket = ''
    res = []
